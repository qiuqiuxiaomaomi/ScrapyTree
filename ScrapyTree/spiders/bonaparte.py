...
from ScrapyTree.items import ScrapytreeItem  # 导入item对应的类，crawlPictures是项目名，items是items.py文件，import的是items.py中的class，也可以import *


class Spdier_pictures(scrapy.spiders.Spider):
    name = "ScrapyTree"  # 定义爬虫名，要和settings中的BOT_NAME属性对应的值一致
        ...
        if (re.match("http://desk.zol.com.cn/fengjing/\d+x\d+/\d+.html", response.url)):  # 如果url能够匹配到需要爬取的url，就爬取
            ...
            for i in range(len(src)):  # 遍历li个数
                ...
                if imgURLs:
                    ...
                    file_name = "%s.jpg"%titles[0]  # 要保存文件的命名

                    path = os.path.join("E:\\picture", file_name)  # 拼接这个图片的路径，我是放在F盘的pics文件夹下

                    type = sys.getfilesystemencoding()
                    print file_name.encode(type)

                    item = ScrapytreeItem()  # 实例item（具体定义的item类）,将要保存的值放到事先声明的item属性中
                    item['name'] = file_name
                    item['url'] = realUrl
